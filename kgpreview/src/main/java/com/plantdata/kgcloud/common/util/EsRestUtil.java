...
public class EsRestUtil {
    ...
    private EsRestUtil(RestClient restClient) {
    ...
    public static EsRestUtil getInstance(HttpHost... hosts) {
        ...
        if (hosts.length == 0) {
            exit("elastic host is empty!");
        }
        return new EsRestUtil(hosts);
    }

    public static EsRestUtil getInstance(String ips, Integer port) {

        String[] ipArr = ips.split(",");
        HttpHost[] hosts = new HttpHost[ipArr.length];
        for (int i = 0; i < ipArr.length; i++) {
            hosts[i] = new HttpHost(ipArr[i], port);
        }
        return new EsRestUtil(hosts);
    }

    public static EsRestUtil getInstance(RestClient restClient) {

        return new EsRestUtil(restClient);
    }

    public static EsRestUtil getInstance(String hostsStr) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String[] rs = restClient.findByScroll("kg_gw_help_test22", "data", null, null, 2, null);
        ...
        while (rs[1].length() > 2) {
            ...
            rs = restClient.findByScroll("kg_gw_help_test22", "data", null, null, 2, scrollId);
        }
