...
@Slf4j
public class OracleOptProvider implements DataOptProvider {
    ...
    public OracleOptProvider(DataOptConnect info) {

        DataSourceBuilder dataSourceBuilder = DataSourceBuilder.create();

        dataSourceBuilder.driverClassName("oracle.jdbc.OracleDriver");
        dataSourceBuilder.url("jdbc:oracle:thin:@" + info.getAddresses().get(0) + ":" + info.getDatabase());
        ...
        this.username=info.getUsername().toUpperCase();
    }
    ...
    private String buildQuery(Map<String, Object> query) {
        ...
        sql.append("SELECT * FROM " +this.username+".\"").append(table).append("\" ");
    ...
    private String buildCount(Map<String, Object> query) {
        ...
        sql.append("SELECT count(*) FROM " +this.username+".\"").append(table).append("\" ");
        for (Map.Entry<String, Object> entry : query.entrySet()) {
            if (Objects.equals(entry.getKey(), "search")) {
                sql.append(" WHERE ");
                Map<String, String> value = (Map<String, String>) entry.getValue();
    ...
    public List<String> getFields() {
        ...
        convertSql(query, "select", "COLUMN_NAME" ,"from" ,"user_tab_columns", "where", "table_name", "=", "'" + table + "'");
    ...
    public List<Map<String, Object>> findWithSort(Integer offset, Integer limit, Map<String, Object> query, Map<String, Object> sort) {
        ...
        if (offset != null && offset >= 0) {
            ...
            if (offset == 0) {
                sql += " where rownum <=" + size;
            } else {
                sql += " where rownum  >" + offset + " and rownum <=" + (size + offset);
            }
        }
    ...
    private String buildOne(String id) {
        ...
        sql.append("SELECT * FROM " +this.username+".\"").append(table).append("\" WHERE id = ").append(id);
    ...
    public List<Map<String, Object>> search(Map<String, String> searchMap, int offset, int limit) {
        ...
        convertSql(select, "select", field, "from", table, "where", builder.toString(), " and  rownum > ", offset + " and rownum <=" + (limit + offset));
