...
public class JpaQuery<T> implements Specification<T> {
     ...
     * and条件
     */
    private List<JpaFilter> andFilters = new ArrayList<>();
    /**
     * or条件
     */
    private List<JpaFilter> orFilters = new ArrayList<>();
    /**
     * 排序属性
     */
    private List<JpaOrder> orders = new ArrayList<>();
    ...
    private Predicate toAndPredicate(Root<T> root, CriteriaBuilder criteriaBuilder) {
        ...
        for (JpaFilter filter : andFilters) {
            if (filter == null) {
                continue;
            }
            String property = filter.getProperty();
            JpaFilter.Operator operator = filter.getOperator();
            ...
            switch (operator) {
                ...
                case gt:
                    ...
                    } else if (String.class.isAssignableFrom(path.getJavaType()) && value instanceof String) {
    ...
    private Predicate toOrPredicate(Root<T> root, CriteriaBuilder criteriaBuilder) {
        ...
        for (JpaFilter filter : orFilters) {
            if (filter == null) {
                continue;
            }
            String property = filter.getProperty();
    ...
    private List<javax.persistence.criteria.Order> toOrders(Root<T> root, CriteriaBuilder criteriaBuilder) {
        ...
        for (JpaOrder order : orders) {
            ...
            JpaOrder.Direction direction = order.getDirection();
            Path<?> path = getPath(root, property);
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        Predicate orRestrictions = toOrPredicate(root, cb);
        if (orRestrictions.getExpressions().size() > 0) {
            restrictions = cb.and(restrictions, orRestrictions);
        }
     ...
     * @return 链式调用
     */
    public JpaQuery and(JpaFilter filter) {
     ...
     * @return 链式调用
     */
    public JpaQuery or(JpaFilter filter) {
     ...
     * @return 链式调用
     */
    public JpaQuery orderASC(String property) {
        this.orders.add(JpaOrder.asc(property));
        return this;
    }
     ...
     * @return 链式调用
     */
    public JpaQuery orderDESC(String property) {
        this.orders.add(JpaOrder.desc(property));
        return this;
    }
    ...
    public List<JpaFilter> getAndFilters() {
        return andFilters;
    }

    public void setAndFilters(List<JpaFilter> andFilters) {
        this.andFilters = andFilters;
    }

    public List<JpaFilter> getOrFilters() {
        return orFilters;
    }

    public void setOrFilters(List<JpaFilter> orFilters) {
        this.orFilters = orFilters;
    }

    public List<JpaOrder> getOrders() {
        return orders;
    }

    public void setOrders(List<JpaOrder> orders) {
        this.orders = orders;
    }
}
